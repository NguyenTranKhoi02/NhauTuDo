var VueSelectOptionHtml = {
    ...
    data: {
        ...
        listStatus: [
            { "class": "orange", "title": "Chờ xác nhận", "value": 0 }
            , { "class": "green", "title": "Đã xác nhận", "value": 1 }
            , { "class": "red", "title": "Hủy Đặt", "value": 2 }
        ],
    },
    init: function () {
        var me = this;
    ...
    initDdlStatusBG: function () {
        ...
        $.each(lstWrapt, function (i, v) {
            let hiddenid = $(v).data('hiddenid');
            htm = `
                <div class ="selectBox select-booking vsoh-wrapt" id="vsoh-status-wrapt-${i}">
                    <div class ="selected ${chosenOption.class} vsoh-show">
                        <div class ="icn ${chosenOption.class} vsoh-show-class"></div>
                        <div class ="text vsoh-show-title">${chosenOption.title}</div>
                    </div>
                    <ul class ="list-select-type list-group">
                    ...
                    <input type="hidden" class ="ddlStatusBG" id="ddl-status-class-${i}" value="${chosenOption.class}" />
                    <input type="hidden" class ="ddlStatusBG" id="ddl-status-value-${i}" value="${chosenOption.value}" />
                    <input type="hidden" class ="ddlColor" id="${hiddenid}" value="${chosenOption.value}" />
                </div>
            `;
            $(v).html(htm);
    ...
    setStatusBG: function (idWrapt, selectedValue) {
        var me = this;
        let iChosen = -1;
        //console.log('me.data.listColor', me.data.listColor);
        // loop to find index chosen in array
